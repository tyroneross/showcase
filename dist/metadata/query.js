export function filterCaptures(captures, query) {
    ...
    if (query.tags_any?.length) {
        results = results.filter((c) => query.tags_any.some((t) => c.tags.includes(t)));
    }
    if (query.feature) {
...
export function groupCaptures(captures, groupBy) {
