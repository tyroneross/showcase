 ...
 */
export async function resolveWindowId(appName) {
...
`;
    try {
        const { stdout } = await execFileAsync("swift", ["-e", swiftCode, appName], { timeout: 15000 });
    ...
    throw new Error(`No running process for '${appName}'. Ensure the app is running.`);
}
/**
 * Take a screenshot of a specific macOS window.
 */
export async function screenCapture(windowId, outputPath) {
    mkdirSync(dirname(outputPath), { recursive: true });
    await execFileAsync("screencapture", ["-l", String(windowId), "-x", outputPath], { timeout: 10000 });
}
/**
 ...
 */
export function startScreenRecording(_windowId, outputPath) {
    mkdirSync(dirname(outputPath), { recursive: true });
    const proc = execFile("screencapture", ["-v", "-x", outputPath], { timeout: 60000 });
    return {
        process: proc,
