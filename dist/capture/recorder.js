...
import { findDevice, startSimulatorRecording, } from "./simulator.js";
import { transcodeToMp4, extractThumbnail, getVideoDuration, isTranscodeNeeded, } from "./transcode.js";
...
export async function recordClip(options) {
    ...
    const duration = Math.min(options.duration ?? config.video.defaultDuration, config.video.maxDuration);
    ...
    try {
        switch (resolved.source) {
            ...
            case "simulator": {
                ...
                if (!device) {
                    throw new Error(`No simulator found for '${resolved.deviceName}'. Run: xcrun simctl list devices`);
                }
                if (device.state !== "Booted") {
                    throw new Error(`Simulator '${device.name}' is not booted. Boot with: xcrun simctl boot "${device.udid}"`);
                }
