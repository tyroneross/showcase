...
function parseRuntime(runtime) {
    if (/watchOS/i.test(runtime))
        return "watchos";
    return "ios";
}
export async function listDevices() {
    const { stdout } = await execFileAsync("xcrun", ["simctl", "list", "devices", "--json"], { timeout: 10000 });
...
export async function findDevice(nameOrUdid) {
...
export async function getBootedDevices() {
    const devices = await listDevices();
    return devices.filter((d) => d.state === "Booted");
}
// --- Screenshot ---
export async function simulatorScreenshot(udid, outputPath) {
    mkdirSync(dirname(outputPath), { recursive: true });
    await execFileAsync("xcrun", ["simctl", "io", udid, "screenshot", "--type=png", outputPath], { timeout: 15000 });
}
// --- Video recording ---
/**
 ...
 * Send SIGINT to stop.
 */
export function startSimulatorRecording(udid, outputPath) {
    mkdirSync(dirname(outputPath), { recursive: true });
    const proc = execFile("xcrun", ["simctl", "io", udid, "recordVideo", "--codec=h264", outputPath], { timeout: 60000 });
