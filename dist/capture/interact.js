 ...
 */
export async function waitForSettle(page, timeout = 5000) {
    try {
 ...
 */
export async function executeStep(page, step, captureDir, index, startTime) {
...
function assertSelector(step) {
    if (!step.selector) {
        throw new Error(`${step.action} step requires 'selector'`);
    }
}
function shouldScreenshot(action) {
    return action !== "wait";
}
